<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Ziya Kök - Projects</title>
    <style>
        body {
            font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
            margin: 0;
            padding: 0;
            background-color: #e9ecef;
            color: #343a40;
        }
        header {
            background: #007bff;
            color: #fff;
            padding: 20px;
            text-align: center;
        }
        header h1 {
            margin: 0;
            font-size: 2.5em;
        }
        header p {
            margin: 5px 0 0;
            font-size: 1.1em;
        }
        nav {
            display: flex;
            justify-content: center;
            gap: 20px;
            padding: 10px 0;
            background: #fff;
            border-bottom: 1px solid #dee2e6;
        }
        nav a {
            color: #007bff;
            text-decoration: none;
            font-weight: bold;
            padding: 10px 20px;
            border-radius: 5px;
            transition: background-color 0.3s, color 0.3s;
        }
        nav a:hover {
            background-color: #007bff;
            color: #fff;
        }
        main {
            padding: 20px;
            max-width: 900px;
            margin: 20px auto;
            background: #fff;
            border-radius: 10px;
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);
        }
        h2 {
            color: #007bff;
            border-bottom: 2px solid #007bff;
            padding-bottom: 5px;
            margin-bottom: 15px;
        }
        p {
            line-height: 1.6;
        }
        ul {
            list-style-type: disc;
            margin: 0;
            padding-left: 20px;
        }
        ul li {
            margin-bottom: 10px;
        }
        footer {
            text-align: center;
            padding: 20px;
            background: #007bff;
            color: #fff;
            position: fixed;
            width: 100%;
            bottom: 0;
        }
    </style>
</head>
<body>
    <header>
        <h1>Ziya Kök - Projects</h1>
        <p>Home / Projects / Develop in Swift / CS-50 Course</p>
    </header>

   <nav>
        <a href="https://ziyarko.github.io/DEVELOPWSWIFT.html">Develop in Swift</a>
        <a href="https://ziyarko.github.io/DOCUMENTATION.html">Documentation</a>
        <a href="https://ziyarko.github.io">Home</a>
        <a href="https://ziyarko.github.io/CS-50COURSE.html">CS-50 Course</a>
    </nav>

  <main>
        <h2>My Projects</h2>
        <p>Welcome to my project page! Here you will find some of the projects I have worked on:</p>

       
<h2>Featured Project</h2>
        <p>Click the link below to visit my Scratch Connect 4 game:</p>
        <a href="https://scratch.mit.edu/projects/1062931090" target="_blank">Connect 4 (Scratch)</a>
    </main>

  <h2>Featured Project</h2>
        <p>Click the link below to visit my Scratch Ping Pong game:</p>
        <a href="https://scratch.mit.edu/projects/1081267471" target="_blank">Ping Pong (Scratch)</a>
    </main>

<footer>
        <p>&copy; 2024 Ziya Kök. All rights reserved.</p>
    </footer>
</body>
</html>

 
